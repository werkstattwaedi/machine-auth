...
Operational config (SDM_BASE_URL, DIVERSIFICATION_SYSTEM_NAME) is loaded
from scripts/.env, generated by scripts/generate-env.ts from the operations
repo. Secrets (DIVERSIFICATION_MASTER_KEY, TERMINAL_KEY) come from
functions/.env.local (dev) or gcloud Secret Manager (--prod).
...
def load_operations_env() -> dict[str, str]:
    """Load operational (non-secret) config from scripts/.env.

    scripts/.env is generated by scripts/generate-env.ts from the operations
    repo (machine-auth-operations/config.jsonc) and provides SDM_BASE_URL,
    DIVERSIFICATION_SYSTEM_NAME, and other deployment-wide settings.
    """
    env_file = _project_root() / "scripts" / ".env"
    if not env_file.exists():
        raise FileNotFoundError(
            f"Operations env file not found: {env_file}\n"
            "Run `npm run generate-env` to generate it from the operations repo."
        )
    values = _parse_env_file(env_file)

    required = ["SDM_BASE_URL", "DIVERSIFICATION_SYSTEM_NAME"]
    for key in required:
        if not values.get(key):
            raise KeyError(
                f"Missing {key} in {env_file}. "
                "Check machine-auth-operations/config.jsonc and re-run "
...
def load_secrets_dev() -> dict[str, str]:
    ...
    env_file = _project_root() / "functions" / ".env.local"
    if not env_file.exists():
        raise FileNotFoundError(
...
def main() -> int:
    ...
    # Load operational config (non-secrets) and secrets
    try:
        ops = load_operations_env()
        if use_prod:
    ...
    system_name = ops["DIVERSIFICATION_SYSTEM_NAME"]
    sdm_base_url = ops["SDM_BASE_URL"]
    ...
    print(f"Secrets loaded ({mode_label}): master_key=...{master_key[-2:].hex()}, "
          ...
          f"system_name={system_name}, sdm_url=https://{sdm_base_url}")
