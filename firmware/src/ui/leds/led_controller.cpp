...
void Section::Render(uint32_t now_ms) {
  switch (effect_.type) {
    ...
    case EffectType::Breathe: {
      uint8_t br = breathe_brightness(now_ms, effect_.period_ms,
                                      effect_.min_brightness,
                                      effect_.max_brightness);
      for (auto i : indices_) owner_->Paint(i, effect_.color, br);
      break;
    }
    case EffectType::Blink: {
      uint8_t phase = static_cast<uint8_t>((now_ms % effect_.period_ms) * 255 / effect_.period_ms);
...
void ButtonSection::Render(uint32_t now_ms) {
  ...
  switch (effect_.type) {
    ...
    case EffectType::Solid:
      br = 255; break;
    case EffectType::Breathe:
      br = breathe_brightness(now_ms, effect_.period_ms,
                              effect_.min_brightness, effect_.max_brightness);
      break;
    case EffectType::Blink: {
      uint8_t phase = static_cast<uint8_t>((now_ms % effect_.period_ms) * 255 / effect_.period_ms);
...
void LedController::InitializeDefaultMapping() {
  ...
  std::vector<float> default_edges = {
      3.0f, 3.0f, 3.0f, 1.0f, 1.0f, 3.0f, 3.0f, 3.0f, 3.0f, 3.0f};
...
void LedController::Tick(uint32_t now_ms) {
  ...
  if (ring_.GetEffect().type == EffectType::Rotate && !ring_.Indices().empty()) {
    ...
    const float total_len = (ring_pos_.size() == n) ? (ring_pos_.back() + ring_wrap_len_) : (float)n;
