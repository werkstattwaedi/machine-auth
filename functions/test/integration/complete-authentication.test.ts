...
describe("handleCompleteAuthentication (Integration)", () => {
  ...
  describe("Session validation", () => {
    it("should reject missing sessionId", async () => {
      const request: CompleteAuthenticationRequest = {
        sessionId: "",
        encryptedNtagResponse: new Uint8Array(16),
      };
    ...
    it("should reject non-existent session", async () => {
      const request: CompleteAuthenticationRequest = {
        sessionId: "nonExistentSession123",
        encryptedNtagResponse: new Uint8Array(16),
      };
    ...
    it("should reject missing encryptedNtagResponse", async () => {
      const request: CompleteAuthenticationRequest = {
        ...
        encryptedNtagResponse: new Uint8Array(0),
      };
  ...
  describe("Permissions handling", () => {
    it("should correctly serialize permissions array to protobufs", async () => {
      ...
      const authRequest: AuthenticateNewSessionRequest = {
        tokenId: { uid: new Uint8Array(Buffer.from(TEST_TOKEN_ID, "hex")) },
        ntagChallenge: new Uint8Array(Buffer.from("0123456789abcdef0123456789abcdef", "hex")),
      };
      ...
      const completeRequest: CompleteAuthenticationRequest = {
      ...
      // Should not get a protobuf packing error
      if (thrownError) {
  ...
  describe("End-to-end authentication flow", () => {
    it("should complete full authentication flow", async function () {
      ...
      expect(authResponse.cloudChallenge).to.be.instanceOf(Uint8Array);
      ...
      expect(completeResponse.result?.$case).to.equal("rejected");
    });
  });
});
