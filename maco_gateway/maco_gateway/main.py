...
def parse_args() -> argparse.Namespace:
    ...
    Env vars are loaded from maco_gateway/.env.<GATEWAY_ENV> if present
    (generated by scripts/generate-env.ts). GATEWAY_ENV defaults to
    "local" (emulator dev); GATEWAY_ENV=staging selects the overlay env
    emitted by `generate-env.ts --env staging` (ADR-0034).
    """
    env_name = os.environ.get("GATEWAY_ENV", "local")
    env_filename = f".env.{env_name}"
    # Check Bazel runfiles first, then workspace directory, then relative path.
    env_candidates = [
        Path(__file__).parent.parent / env_filename,  # Bazel runfiles
    ]
    workspace = os.environ.get("BUILD_WORKSPACE_DIRECTORY", "")
    if workspace:
        env_candidates.append(Path(workspace) / "maco_gateway" / env_filename)
