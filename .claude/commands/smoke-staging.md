...
a list of assertions: the scripted suite (`npm run smoke:staging`) already
covers "does it still work"; you are here for what nobody thought to check — an
uninvited dialog, a wrong name, broken layout, odd copy, an invoice that looks
off.
...
- Arguments: $ARGUMENTS (e.g. "only A and D", "include the mobile viewport").
  With none, run all scenarios.
