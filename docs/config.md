...
| `VITE_ENV_LABEL` | Prefix for the browser tab title (`web.envLabel`; the kiosk window/tray title reads the same key). Empty in production | `[staging]` |
...
### Keep-warm pings (ADR-0037)

`WARM_SCHEDULE` (operations config `gateway.warmSchedule`, e.g.
`Mon-Sun 08-22`) makes the gateway ping the Firebase `api` function's
`/ping` route every 10 minutes inside the weekly window, so the first tag
check-in of the day doesn't pay a cold start. Empty or absent disables the
pinger. See [ADR-0037](adr/0037-keep-warm-pings.md).
...
Terminal registration is manual: create a `maco/{deviceId}` doc in Firestore
(fields `name`, `hwRevision`) with the Particle device ID as the doc ID. The
former admin-API import from Particle Cloud was retired together with the
`PARTICLE_TOKEN` secret.
...
- [ ] Terminals registered as `maco/{deviceId}` docs in Firestore
