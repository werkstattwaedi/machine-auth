...
**Amendment (2026-08-11, issue #595): kiosk sign-up and kiosk onboarding.** Visitors kept hitting a dead end at the kiosk ("registriere dich zuerst auf deinem eigenen Gerät"), so both account *creation* and the first-login *welcome onboarding* now run at the kiosk — without weakening the session model. Three pieces:

- **Sign-up (`signupKiosk`)** — a truly new e-mail gets the regular sign-up dialog; the kiosk-bearer-gated callable (`functions/src/checkout/signup_kiosk.ts`) verifies the login code, validates the form server-side, creates the Firebase Auth user **and** the `users` doc (terms accepted in the form) with the admin SDK, then mints the same ephemeral `actsAs` principal via `mintKioskSessionToken`. It refuses any e-mail that already has a `users` doc (an imported member's profile must never be overwritten by freshly typed data — pre-check plus a `create()` write).
- **Onboarding for unclaimed/imported members** — `verifyLoginCodeKiosk` now requires only a `users` doc, not accepted terms (a badge tap never checked terms either, so this adds no authority). The wizard then overlays the same 4-step "Willkommen" dialog (`kiosk-welcome-onboarding.tsx`, reusing the shared `WelcomeOnboardingView`), reading the doc via the actsAs read rule. Its writes go through **`completeOnboardingKiosk`** (`functions/src/checkout/complete_onboarding_kiosk.ts`): the established kiosk session is the credential — the callable requires the `tagCheckout` + `actsAs` claims and writes only that user's profile fields/terms via the admin SDK. The door closes with onboarding: profile writes are rejected once terms are accepted, and `termsAcceptedAt` is **write-once** (a routine kiosk session can never restamp the consent record). Firestore rules still reject direct users-doc writes from synthetic principals; the callable is the only door. Step 4 swaps the member-area links (family management etc. — unreachable for actsAs sessions) for the instructions-email offer.
- **Account-instructions email** (`self-checkout-account-instructions` Resend template; 1h per-account throttle via `users.accountInstructionsSentAt`) — sent automatically after a kiosk sign-up and offered in the kiosk onboarding's step 4, so the account isn't stranded on a shared terminal: it explains how to sign in on your own device later. The `sendAccountInstructions` callable is **session-bound**: it requires the `actsAs` claims and derives the recipient from that user's doc — no e-mail in the payload, so it is neither an account-existence oracle nor a mail-arbitrary-addresses trigger.
