...
function main() {
  ...
  // Environment overlay mode (`--env <name>`, ADR-0034): deep-merge
  // `config.<name>.jsonc` over the base config and emit ONLY the
  // deploy-facing, env-suffixed files (functions/.env.<projectId>,
  // web/apps/*/.env.<name>). Non-destructive — never rewrites the prod
  // `.env.production` / `.env.local` / `.firebaserc`.
  const envName = parseEnvName(args);
  if (envName) {
    generateEnvOverlay({ projectRoot, configDir, prodConfig, envName });
    return;
  }
...
/** Parse `--env <name>` or `--env=<name>` from argv; null if absent. */
function parseEnvName(args: string[]): string | null {
...
/**
 * Emit the env-suffixed deploy files for a named environment overlay
 * (`--env staging`). Deep-merges `config.<name>.jsonc` over the base
 * config, then writes:
 ...
 */
function generateEnvOverlay(opts: {
  projectRoot: string;
  configDir: string;
  prodConfig: Record<string, unknown>;
  envName: string;
}): void {
  const { projectRoot, configDir, prodConfig, envName } = opts;
  const overridePath = resolve(configDir, `config.${envName}.jsonc`);
  if (!existsSync(overridePath)) {
    console.error(
      `Error: ${overridePath} not found.\n\n` +
        `The --env ${envName} overlay needs a config.${envName}.jsonc in the ` +
        `operations repo holding just the values that differ from config.jsonc ` +
        `(at minimum firebase.projectId, the target project's SDK config, and ` +
        `the web.*Domain values).`
    );
    process.exit(1);
  }
  ...
  const envConfig = deepMerge(
    prodConfig,
    parseJsonc(readFileSync(overridePath, "utf-8"))
  );

  const projectId = resolveValue(envConfig, "firebase.projectId");
  if (!projectId) {
    console.error(
      `Error: firebase.projectId is required (config.jsonc or config.${envName}.jsonc)`
    );
    process.exit(1);
  }

  const header = `# Generated by scripts/generate-env.ts --env ${envName} — do not edit`;
  const files: OutputFile[] = [
    ...
    {
      path: `web/apps/checkout/.env.${envName}`,
      source: "production",
      header,
      sections: [
        { comment: "# Firebase", vars: VITE_FIREBASE },
    ...
    {
      path: `web/apps/admin/.env.${envName}`,
      source: "production",
      header,
      sections: [
        { comment: "# Firebase", vars: VITE_FIREBASE },
  ...
  for (const file of files) {
    const content = generateEnvContent(envConfig, file);
    writeFileSync(resolve(projectRoot, file.path), content);
    console.log(`  ✓ ${file.path}`);
  }
