...
**Catalog import from Mario's pricelist xlsx** (admin `/materials/import`): Mario maintains one Excel workbook with a `… BL` sheet per workshop (Holz/Metall/Keramik/Textil). The import contract is five explicit columns — `Code` (stable 4-digit identity, **never reused/renumbered**), `Name`, `Kategorie`, `Unterkategorie`, `Einheit` (→ pricing model) — plus Mario's existing `Preis Einheit Verkauf` column. Columns are matched by *header text*, not position, so Mario can rearrange his calc columns freely. Pipeline: `previewCatalogImport`/`applyCatalogImport` (catalogCall) parse the upload with **exceljs** (which reads the *cached* result of the price formulas), normalise+diff via `@oww/shared` `catalog-import.ts` (create/update/unchanged/retire by code; retirement scoped to imported workshops, never touches `type:"machine"` items), and apply in batched writes (updates preserve member price / extra variants). Two gotchas: (1) a workbook written by a tool that doesn't evaluate formulas (e.g. the bootstrap below) has **no cached price** — the preview surfaces a "open in Excel and save" hint instead of 200 per-row errors; (2) the one-time bootstrap that backfilled codes into Mario's file lives at `scripts/augment-pricelist-bootstrap.py` (Python/openpyxl, to preserve his formulas while appending columns).
