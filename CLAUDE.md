...
**Member identity** (ADR-0043, `functions/src/auth/identity.ts`): `users/{uid}` is canonical, Firebase Auth follows it. Never resolve a member with `getUserByEmail` / `createUser` directly — login paths go through `resolveLoginUid` (users doc first; recreates a missing Auth record under the same uid incl. its `admin` claim; never signs a stale Auth e-mail into a member's account), everything else through `ensureAuthIdentity` / `findUserDocByEmail`. `users.email` is not client-writable (rules pin it to the token e-mail on create, to its prior value on update) — the admin profile tab changes it via the `updateUserEmail` callable. The `syncAuthIdentity` trigger pushes doc e-mail → Auth and **unlinks the Auth phone whenever it differs from `users.phone`**, so any test or script that links an Auth phone must set `users.phone` too (e2e: `linkPhone` helper). The doc lookup is an exact-match query and the phone comparison plain string equality — stored values must stay normalised (e-mail trim+lowercase, phone E.164); `scripts/check-user-identity-fields.ts --prod` verifies that (read-only, prints uids only) and is a pre-deploy gate. `isBareAuthRecord` is the guard of a deletion path — don't loosen a term without updating `docs/disaster-recovery.md` and the per-term tests in `identity-resolve.test.ts`.
