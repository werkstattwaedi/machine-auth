...
**Bill cancellation & corrections** (ADR-0042, `functions/src/invoice/correct_checkouts.ts`): bill numbers are stored as `base × 10 + revisionDigit` — always render through `formatBillReference`, never print the raw number, and seed test fixtures with the shifted value (`42000010` is `RE-4200001`, `42000011` its first correction `RE-4200001-1` — the printed suffix is the stored digit, matching the QR payload's last digit). Cancelled checkouts/bills stay in place (`status: "cancelled"` / `cancelledAt`, both linked via `supersedes…`/`supersededBy…`); legacy docs lack these fields, so "not cancelled" is always a client-side check, never a Firestore `== null` filter. A correction is one transaction including the Sammelrechnung revision (`aggregateBelegeIntoInvoice` is shared with `monthlyBillRun`), and one mail per top-level bill (replacement Belege inside a revision ride along as attachments). Cancelled + replacement checkouts carry `statsFlushedAt: null` until the 05:00 export's final pass flushes them. Deploy order: `setup-bigquery.ts`, then **functions, then** `migrate-bill-numbers.ts` right after (minting is blocked in between; migrating first would let the old code mint un-shifted numbers), then hosting — see `docs/deployment-checklist.md`.

**Data protection & statistics** (ADR-0038/0039, `docs/data-protection.md`): Firestore is the operational store with a 3-year retention; BigQuery dataset `stats` holds pseudonymized statistics that survive deletion (HMAC subject keys via the `STATS_SUBJECT_SALT` secret; `STATS_DATASET` param). The subject-data map (`functions/src/privacy/subject_data_map.ts`) is the per-collection policy register — a coverage test fails CI if an audited collection lacks an entry. Admin DSAR tooling runs through `scripts/privacy-cli.ts` (`report | erase | trim`, always `--dry-run` first) against the `authCall` dispatcher. Two gotchas: (1) every new `defineString`/`defineSecret` must be wired through `scripts/generate-env.ts` (and `FUNCTION_SECRET_NAMES` in `scripts/port-block-secrets.ts` for secrets) or the functions emulator prompts interactively and hangs every emulator-backed test run; (2) never delete checkout/bill/usage docs outside the privacy engines — they guard on export watermarks (`export_state/*`) so statistics are flushed before deletion.

**Kiosk step-up** (ADR-0041, `functions/src/checkout/kiosk_elevation.ts`): a kiosk `actsAs` session reaches `/account/*` and the membership callables only while its `elevatedUntil` claim (epoch ms) is in the future. Badge taps mint without it; e-mail/SMS code sign-ins mint with it; a badge session steps up via `requestKioskElevation`/`verifyKioskElevation` (server picks the stored e-mail) or the SMS path through `exchangeKioskSession({ expectedUserId })`. Rules use `isElevatedActsAs()`, callables `requireElevatedActsAs`/`callerUserRef` — never gate a kiosk session on `request.auth.uid` (synthetic). TTL is `KIOSK_ELEVATION_TTL_MS` (`defineString`, default 15 min, empty = default). The web auth context listens on `onIdTokenChanged` because the step-up re-signs in with the same uid.

**Member identity** (ADR-0043, `functions/src/auth/identity.ts`): `users/{uid}` is canonical, Firebase Auth follows it. Never resolve a member with `getUserByEmail` / `createUser` directly — login paths go through `resolveLoginUid` (users doc first; recreates a missing Auth record under the same uid incl. its `admin` claim; never signs a stale Auth e-mail into a member's account), everything else through `ensureAuthIdentity` / `findUserDocByEmail`. `users.email` is not client-writable by anyone, **admins included** (rules pin it to the token e-mail on create, to its prior value on update in every branch) — the admin profile tab changes it via the `updateUserEmail` callable, and any new admin tooling must too. The `syncAuthIdentity` trigger pushes doc e-mail → Auth and **unlinks the Auth phone whenever it differs from `users.phone`**, so any test or script that links an Auth phone must set `users.phone` too (e2e: `linkPhone` helper). The doc lookup is an exact-match query and the phone comparison plain string equality — stored values must stay normalised (e-mail trim+lowercase, phone E.164); `scripts/check-user-identity-fields.ts --prod` verifies that (read-only, prints uids only) and is a pre-deploy gate. `scripts/privacy-cli.ts audit-identity [--fix]` is the doc↔Auth consistency check + backfill (`functions/src/auth/identity-audit.ts`; uids only, exit 4 while findings remain) — run it after any restore, since PITR brings back users docs but not Auth records. `isReclaimableBareRecord` (shape via `isBareAuthRecord` + idle ≥ 2 h) is the guard of a deletion path — don't loosen a term without updating `docs/disaster-recovery.md` and the per-term tests in `identity-resolve.test.ts`; tests need aged records for it (`importIdleBareUser` in `functions/test/emulator-helper.ts`). `signInWithGoogle` may drop its session again (guard), so effects that route a signed-in, doc-less principal must wait for `googleSignInPending`.

**Staging-only `mintTestTap`** (`functions/src/testing/mint_test_tap.ts`, ADR-0034 amendment): mints the `picc`/`cmac` of a badge tap for a *virtual* tag so the post-deploy smoke test can run kiosk flows without a reader or the tag keys. Staging shares `TERMINAL_KEY`/`DIVERSIFICATION_MASTER_KEY` with prod, so an unrestricted minter could forge a tap for any real badge — never loosen its three guards: the export in `index.ts` is conditional on `GCLOUD_PROJECT === "oww-maco-staging"` (firebase-tools sets it to the deploy target during discovery) and the handler 404s elsewhere; `invoker: "private"` + the staging kiosk bearer; UIDs must start with `f0` (real NXP tags start with `04`). `KIOSK_BEARER_KEY` is the one secret that is **per environment**, not shared. The minting crypto lives once in `src/ntag/sdm_mint.ts`; `test/test-sdm-helper.ts` re-exports it. Its sibling `uploadTestUsage` (`upload_test_usage.ts`) feeds one machine session into the real `handleUploadUsage`; it shares the first two guards (`testing/staging_guard.ts`) and its own third is "only accounts whose `users.email` starts with `smoke.testing+`". A new staging-only hook needs all of: the shared guard, the conditional export, and its name in the `deploy.sh prod` absence check.
...
- `authentications.ttlAt`: created + 5 min for in-progress auth; cleared (set to `null`) on successful completion. Completed records are retained until the yearly 3-year retention trim (ADR-0038), not indefinitely.
...
**Staging smoke run (agent, by choice):** the emulator cannot show missing indexes, races hidden by instant commits, IAM/secrets/origin config, wrong-project bundles or real mail delivery — each has shipped a bug past the full suite — and no scripted check anticipates an uninvited dialog or an invoice without a recipient. So the smoke test is an **agent walking the deployed staging apps and judging what it sees**: `/smoke-staging` follows `../machine-auth-operations/smoke/RUNBOOK.md` (private ops repo — it carries the smoke mailbox's credential) through a persistent browser driver (`npm run agent -- …`), opens every screenshot, the rendered mails and the invoice PDF pages, and writes a report with a verdict. There is deliberately **no scripted smoke suite** — add scenarios to the runbook as *intent* ("who you are, what you try to get done, what good looks like"), never as selectors. Staging-only test hooks back it: `mintTestTap` (virtual badge taps) and `uploadTestUsage` (a machine session through the real usage handler). When a human answers one of its "is this intended?" questions, the answer goes into the runbook's "Known facts".
...
**The Firestore emulator commits too fast to surface create-then-listen races.** A rule that `get()`s a parent doc (e.g. `checkouts/{id}/items` reads) is evaluated when a listener is *registered*, and the SDK's latency compensation surfaces a freshly-created parent to queries before the server has committed it. A listener opened off that local snapshot can reach the server first, get `permission-denied`, and — listener errors being terminal — stay dead until reload, while every write still lands. Locally the commit always wins, so the suite stays green. When a subcollection listener keys off a parent the same client just created, gate it on the create's promise (see `useCheckoutCreateLatch` in the checkout wizard context; staging incident 2026-09-02).
